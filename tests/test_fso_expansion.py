...
def test_load_paths_empty_csv(setup_csv, setup_env):
    pm = FsoExpansion(csv_name=setup_csv.name, csv_root_dir=str(setup_env), csv_dir_name='csv')
...
def test_create_file_no_save(setup_csv, setup_env):
    ...
    assert new_file.exists() and new_file.is_file(), "File should be created."
    assert not any(p['name'] == "test_file_nosave_txt" for p in pm.paths), "Path should not be saved in the paths list."
