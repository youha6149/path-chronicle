...
def test_list_paths_entry(setup_csv, setup_env):
    run_command(f"poetry run pcmkdir test_dir {setup_env} --description 'Test directory' --csv_name {setup_csv.name} --csv_root_dir {setup_env} --csv_dir_name csv", cwd=PROJECT_ROOT)
    run_command(f"poetry run pctouch test_file.txt {setup_env} --description 'Test file' --csv_name {setup_csv.name} --csv_root_dir {setup_env} --csv_dir_name csv", cwd=PROJECT_ROOT)
    ...
    for header in expected_headers:
        assert header in lines[0], f"Expected header '{header}' not found in output: {lines[0]}"
